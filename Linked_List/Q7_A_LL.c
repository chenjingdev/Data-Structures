...
void RecursiveReverse(LinkedList *ptrHead);
...
int main()
{
	...
	while (c != 0)
	{
		...
		case 2:
			...
			// ll.head = RecursiveReverse(ll.head);
			// printf("The resulting linked list after reversed the given linked list is: ");
			// printList(&ll);
			// removeAllItems(&ll);

			RecursiveReverse(&ll); // You need to code this function
			printf("The resulting linked list after reversed the given linked list is: ");
			printList(&ll);
			removeAllItems(&ll);
			break;
...
void RecursiveReverse(LinkedList *ll)
{
	ListNode *prev = NULL;
	...
	ll->head = prev; // 직접 ll의 head를 업데이트
}
