...
int hasGreatGrandchild(BTNode *node)
{
    // 노드가 NULL인 경우, 재귀 호출을 중단합니다.
    if (node == NULL)
    {
        return 0; // 현재 노드에서 증조손노드까지의 깊이가 부족합니다.
    }

    // 깊이를 계산하는 부분 (노드에서 3단계 아래에 자식이 존재하는지)
    ...
    // 현재 노드에서 증조손노드까지 3단계 아래가 존재하는지 확인
    if (maxDepth >= 4)
    {
        printf("%d ", node->item); // 증조손노드가 있는 노드의 값을 출력
    }
