...
int maxHeight(BTNode *node)
{
    if (node == NULL)
    {
        return 0; // 노드가 NULL이면, 높이는 0입니다.
    }
    else
    {
        int leftHeight = maxHeight(node->left);   // 왼쪽 자식의 높이를 계산
        int rightHeight = maxHeight(node->right); // 오른쪽 자식의 높이를 계산

        // 두 자식 중 더 큰 높이에 1을 더해서 반환
